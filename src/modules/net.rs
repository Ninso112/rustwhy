...
fn check_ping_latency(report: &mut DiagnosticReport, host: &str) {
...
fn check_dns_resolution(report: &mut DiagnosticReport, host: &str) {
...
fn collect_interface_stats(report: &mut DiagnosticReport) {
...
impl DiagnosticModule for NetModule {
    ...
    async fn run(&self, config: &ModuleConfig) -> Result<DiagnosticReport> {
        ...
        check_ping_latency(&mut report, host);

        // DNS resolution (try getent or host)
        check_dns_resolution(&mut report, host);

        // Interface stats from /proc/net/dev
        collect_interface_stats(&mut report);
